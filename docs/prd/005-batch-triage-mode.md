...
11. As a maintainer, I want excluded issues to have their triage label removed silently by default (matching the event-driven pipeline's silent-skip semantics), with an opt-in config flag (`batch.comment_on_exclude`) to post an explanatory comment when desired, so that issues labelled `skip-ai` or `wontfix` are kept AI-free while the issue does not re-enter the batch queue.
...
- **Config schema additions:** Config is parsed by a hand-written snake_case→camelCase mapper in `src/config/loader.ts` (there is no schema validator), so this touches three files: `RepoConfig` in `src/types.ts`, `DEFAULT_CONFIG` in `src/config/schema.ts`, and the mapping in `loader.ts`. A new optional `batch` object is added. In YAML (snake_case) it is:
   ...
   - `batch.comment_on_exclude` (boolean, default: `false`) — when `true`, posts an explanatory comment on excluded issues; when `false` (default), excluded issues are drained silently (label removed, no comment)

   These map onto `RepoConfig.batch.{ triageLabel: string; batchLimit: number; commentOnExclude: boolean }` (camelCase) following the existing convention (e.g. `max_issue_length` → `maxIssueLength`).
...
- **Exclude rules apply:** Before processing each fetched issue, the extracted `shouldExclude` logic (excluded users, excluded labels) is applied. Issues that match are **drained**: the triage label is removed (so the issue is not re-queued on the next run). An explanatory comment via `postExcludeRemovalComment` is posted **only when `batch.comment_on_exclude` is `true`** (default: `false`). This aligns with the event-driven pipeline's silent-skip semantics: excluded issues are silently bypassed in `runPipeline`, and by default receive no comment in `runBatchPipeline` either. The label removal is always performed to prevent re-queueing. Drain failures (label removal or comment post errors) are swallowed with a warning and do not count against `issuesFailed`. Excluded issues are not processed and do not count against either `issuesProcessed` or `issuesFailed`.
